...
adopt_cluster_scoped_resources() {
    ...
    if [ -n "$current_ns" ] && [ "$current_ns" != "$ns" ]; then
        error "OpenShell cluster resources are already owned by a release in namespace $current_ns. Remove that release or use its namespace."
        exit 1
    fi
}
...
deploy_http_echo() {
    local ns="$1"
    if oc -n "$ns" get svc http-echo &>/dev/null; then
        info "http-echo service already exists, skipping"
        return 0
    fi
    step "Deploy in-cluster POST echo service"
    oc -n "$ns" create deployment http-echo \
...
test_curl() {
    ...
    if [ "$code" = "200" ] || [ "$code" = "401" ] || [ "$code" = "301" ] || [ "$code" = "302" ]; then
        ...
        printf "  ${YELLOW}[ERROR]${NC}   %-55s -> HTTP %s (no proxy-denial evidence)\n" "$label" "${code:-ERR}"
        return 2
    fi
}
